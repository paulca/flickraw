...
To use Typhoeus with flickraw, pass a block to any of the supplied api methods. eg.
    
    flickr.photos.getInfo(:photo_id => '3839885268') do |info|
      puts info
    end

    flickr.photos.getInfo(:photo_id => '3839885269') do |info|
      puts info
    end
    
    flickr.photos.getInfo(:photo_id => '3839885270') do |info|
      puts info
    end
    flickr.hydra.run

Hydra is Typhoeus's queuing system. When you call `flickr.hydra.run`, all three requests are made in parallel, through libcurl : much faster than Net::HTTP.
